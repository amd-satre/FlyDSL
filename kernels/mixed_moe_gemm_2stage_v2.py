...
from aiter.ops.flydsl.kernels.mfma_epilogues import c_shuffle_epilog, default_epilog
    ...
    if True:
        ...
        def moe_gemm1(
            ...
            def ptr_buffer_resource(ptr, num_records_bytes):
                # our flyc.compile passes tensor args as MEMREFS (not raw pointers),
                # so build the buffer resource directly from the memref (as our v1
                # kernel does) instead of aiter's ptrtoint + create_from_addr, which
                # needs a raw pointer. Single choke point -> all arg resources +
                # their buffer_load/store sites work unchanged.
...
def compile_mixed_moe_gemm2(
    *,
    ...
    if const_expr(a_dtype not in ("fp8", "fp4", "fp6")):
        raise ValueError(f"a_dtype must be one of ('fp8','fp4','fp6'), got {a_dtype!r}")
    if const_expr(b_dtype not in ("fp8", "fp4")):
        raise ValueError(f"b_dtype must be one of ('fp8','fp4'), got {b_dtype!r}")

    is_f8_a = a_dtype == "fp8"
    is_f4_a = a_dtype == "fp4"
    is_f6_a = a_dtype == "fp6"  # W4A6: MXFP6-E2M3 act (1 B/elem like fp8; 24B codes + 8B pad per K-chunk; cbsz=2)
    is_f4_b = b_dtype == "fp4"
    is_f8_b = b_dtype == "fp8"
    ...
    a_elem_vec_pack = 2 if is_f4_a else 1
    # fp6 A: per-lane K-pack is 32 B (24 B codes + 8 B zero pad) vs 16 B for fp4/fp8.
    ...
    if const_expr(True):
        ...
        def moe_gemm2(
            ...
            if const_expr(is_f4_a or is_f8_a or is_f6_a):
                # #3476: use 256-padded K/32 to match host scale padding.
                # fp6 uses E8M0 microscaling like fp4/fp8 (NOT the fp16-scale else).
            ...
            def moe_gemm2_then_body():
                ...
                # fp6 uses a 32 B/lane K-pack (24 B codes + 8 B pad) vs 16 B for
                # fp4/fp8 -> matches our v1 fp6 layout (a2 read at col_base+16).
                col_offset_base = lane_div_16 * arith.constant(
                    a_per_lane_kpack_bytes, index=True
                )
                        ...
                        for mi in range_constexpr(m_repeat_packed):
                            ...
                            for imxdl in range_constexpr(pack_M):
                                ...
                                if const_expr(is_f8_a):
                                    ...
                                    # fp6: 24 B codes = fp8's layout minus its last i64.
                                    # a0,a1 at col_base (16 B) + a2 = first i64 at
                                    # col_base+64 (aiter's 2nd-half stride) + 8 B zero
                                    # pad; cbsz=2 MFMA ignores the pad.
                                    a2, _ = lds_load_packs_k64(
                                        curr_row_a_lds, col_base + 16, lds_base
                                    )
                                    a128_list[imxdl] = pack_i64x4_to_i32x8(
                                        a0, a1, a2, c0_i64
                                    )
                ...
                # arg_out is a memref under our flyc: get its base byte-address as an
                # index (the standard MLIR way, same as create_buffer_resource does
                # internally) instead of aiter's ptrtoint on a raw pointer. Only the
                # raw-address store branches consume it; use_buf_atomic uses out_rsrc.
                out_base_idx = buffer_ops.extract_base_index(arg_out, 1)
